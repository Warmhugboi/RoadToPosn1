...
|  | or | เป็นจริงก็ต่อเมื่อมีตัวใดตัวหนึ่งเป็นจริง |
...
| vec[i] / .at(i) | เข้าถึงตำแหน่งที่ i | O(1) |
...
สำหรับ `set` เราไม่สามารถเข้าถึงตำแหน่งแบบ `array` หรือ `vector` ได้แต่เราจะนำไปใช้งานแบบอื่น
...
`เกล็ดความรู็!!` เราสามารถใช้ `sort()` กับชุดข้อมูลที่เป็นตัวอักษรหรือข้อความได้ ซึ่งจะมีการเรียงตามค่า Ascii เลย เช่น
...
เรามาพูดถึงเรื่องการ `search` คือ สมมติเรามีชุดข้อมูลแล้วหาว่าตัวที่เราต้องการอยู่ตำแหน่งไหน \
อย่างเช่น `0,8,5,2,7` ถามว่า `5` อยู่ที่ตำแหน่งที่เท่าไหร่ ก็คงต้องตอบว่า `index` ที่ `2` แล้วลองนึกดูว่าถ้าเรามีข้อมูลเป็นพันล้านตัว กว่าเราจะหาเจอหรือกว่าโปรแกรมจะหาเจอ คงใช้เวลาตู้มต้ามเลย
...
สมมติเราอยากหาว่า `33` อยู่ที่ตำแหน่งเท่าไหร่

- ครั้งที่ 1 เริ่มการแบ่งครึ่งครั้งแรก `index <sub>กึ่งกลาง</sub> = (0+9)/2 = 4.5 ปัดลงเป็น 4` มีค่าเท่า `20` ซึ่งน้อยกว่า `33` เราเลยสามารถตัดฝั่งซ้ายรวมถึงตัวมันเอง `20` ออก
...
- ครั้งที่ 4 จะเห็นว่าเหลือข้อมูลตัวเดียวแล้ว ค่ากลางก็คือ `33` ซึ่งเท่ากับ ค่าที่เราต้องการหา ก็ถือว่าจบการทำงาน ก็จะได้ค่า index ตำแหน่งนี้มา

ถ้ามีข้อมูลแค่นี้อาจจะไม่ช่วยให้เร็วขึ้นมาก แต่ถ้ามีข้อมูลเป็น `1,000,000 ตัว` จะแบ่งครึ่งเป็น `500,000 ตัว` แบ่งอีกเป็น `250,000 ตัว` ไปเรื่อยๆจนเจอ ซึ่งจะช่วยลดเวลาได้เป็นอย่างมาก 
