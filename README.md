...
| .erase() | ลบค่าตรงที่ค่าในวงเล็บชี้อยู่ โดยค่าในวงเล็บต้องเป็น iterator | O(n) |
| .substr(start,steps) | การค่า substring หรือ string ย่อย โดยที่ start คือตำแหน่งเริ่มต้นของ substring ที่เราอยากได้ โดยที่ตำแหน่งเป็น index และ steps คือเราต้องการ substring ที่มีความยาวเท่าไหร่ (เริ่มที่ start นับไปอีก steps ตัว) | O(n) |
